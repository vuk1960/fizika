...
P_MIN, P_MAX = 30000.0, 300000.0 # Pascalban
T_MIN, T_MAX = 100.0, 800.0      # Kelvinben

# --- MEMÓRIA (Session State) ---
if 'V' not in st.session_state:
    st.session_state.V = 0.0246
    st.session_state.P = 100000.0
    st.session_state.T = 300.0
    st.session_state.n = 1.0
    st.session_state.history = {'V': [0.0246], 'P': [100000.0], 'T': [300.0]}
...
v_num = col_v2.number_input("V_num", V_MIN, V_MAX, v_sl, format="%.4f", label_visibility="collapsed")

# Nyomás (10^5 skálázás mindenhol)
st.sidebar.write("Nyomás (p [10⁵ Pa])")
col_p1, col_p2 = st.sidebar.columns([2, 1])
p_sl = col_p1.slider("p_slider", P_MIN/1e5, P_MAX/1e5, float(st.session_state.P/1e5), 0.1, label_visibility="collapsed")
p_num = col_p2.number_input("p_num", P_MIN/1e5, P_MAX/1e5, p_sl, step=0.1, format="%.1f", label_visibility="collapsed")
...
t_num = col_t2.number_input("T_num", T_MIN, T_MAX, t_sl, step=1.0, label_visibility="collapsed")

# Értékek begyűjtése (Pascalra váltás a számításhoz)
temp_V, temp_P_Pa, temp_T = v_num, p_num * 1e5, t_num

# FIZIKAI LOGIKA + HIBAVÉDELEM (CLIP)
if new_n != st.session_state.n:
    st.session_state.n = new_n
    temp_P_Pa = np.clip(calc_P(temp_V, temp_T, new_n), P_MIN, P_MAX)
...
def plot_it(ax, x, y, xl, yl, title, xlim, ylim, scale_y=1, scale_x=1):
...
# Nyomás tengelyek 10^5 skálázással
plot_it(ax_pv, h['V'], h['P'], "V [m³]", "p [10⁵ Pa]", "p-V diagram", (V_MIN, V_MAX), (P_MIN/1e5, P_MAX/1e5), scale_y=1/1e5)
plot_it(ax_vt, h['T'], h['V'], "T [K]", "V [m³]", "V-T diagram", (T_MIN, T_MAX), (V_MIN, V_MAX))
plot_it(ax_pt, h['T'], h['P'], "T [K]", "p [10⁵ Pa]", "p-T diagram", (T_MIN, T_MAX), (P_MIN/1e5, P_MAX/1e5), scale_y=1/1e5)
...
ax_pist.plot([st.session_state.V, st.session_state.V + 0.025],, color='gray', lw=10)
